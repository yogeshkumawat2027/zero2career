...
function GovtUpdatesManagement() {
  ...
  const fetchUpdates = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/govt-updates`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingUpdate
        ? `${API_BASE_URL}/api/v1/govt-updates/${editingUpdate.slug}`
        : `${API_BASE_URL}/api/v1/govt-updates`;
  ...
  const handleDelete = async (slug) => {
    ...
    const token = localStorage.getItem('adminToken');
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/govt-updates/${slug}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div>
                <h1 className="text-lg sm:text-2xl font-bold text-gray-900">Govt Updates Management</h1>
                <p className="text-gray-600 text-xs sm:text-sm">Manage government job updates</p>
              </div>
            </div>
            <button
